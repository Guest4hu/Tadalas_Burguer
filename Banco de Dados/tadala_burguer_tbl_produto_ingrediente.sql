...
CREATE TABLE `tbl_produto_ingrediente` (
  ...
  `qtd_por_unidade` decimal(12,3) unsigned NOT NULL,
  `unidade_medida` varchar(10) NOT NULL,
  `criado_em` datetime NOT NULL DEFAULT current_timestamp(),
  `atualizado_em` datetime DEFAULT NULL,
  `excluido_em` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_bom` (`id_produto`,`id_ingrediente`),
  KEY `fk_bom_ing` (`id_ingrediente`),
  CONSTRAINT `fk_bom_ing` FOREIGN KEY (`id_ingrediente`) REFERENCES `tbl_ingredientes` (`id_ingrediente`),
  CONSTRAINT `fk_bom_prod` FOREIGN KEY (`id_produto`) REFERENCES `tbl_produtos` (`id_produto`)
