...
export async function renderizarItensDoPedido(pedidoId, usuarioId) {
   ...
   dadosPedidos.push({
      ...
      produtosAtivos.produtos.forEach(produto => {
         html += `<option value="${produto.produto_id}@${produto.preco}@${pedidoId}`
         if (dados.tipoPedido.tipo_pedido == 3) {
            html += `@${usuarioId}`
         }
         html += `" data-pedidoId="${pedidoId}">${produto.nome}</option>`
      });
      html += `
      </select>
                  </td>
                 ...
                 <div class="pedido-detalhes" style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;"> 
                  `
                   if (dados.tipoPedido.tipo_pedido == 3) {
         ...
         <li><strong>Rua:</strong> ${enderecoDados.endereco.rua}, No ${enderecoDados.endereco.numero}</li>
      ...
      html += `
               ...
               <li><strong>Status:</strong><select id="status_pagamento_id${pedidoId}" class="select_status selectMetodoStatus" style="max-width:240px;">`;
               dados.statusPagamento.forEach(status => {
                  html += `<option value="${status.id}">${status.descricao}</option>`;
               });
               html += `</select></li>
               <li><strong>Método:</strong><select id="pagamentoMetodo${pedidoId}" class="select_status selectMetodoPagamento" style="max-width:240px;">`;
