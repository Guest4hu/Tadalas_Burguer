...
class Funcionarios {
    ...
    public function paginacaoFuncionarios(int $pagina = 1, int $por_pagina = 10): array{
        ...
        $dataQuery = "SELECT f.funcionario_id, u.nome, u.email, f.salario,c.cargo_descricao, sf.descricao FROM tbl_funcionarios AS f INNER JOIN tbl_usuario as u on f.usuario_id = u.usuario_id INNER JOIN dom_status_funcionario as sf ON f.funcionario_id = sf.id INNER JOIN dom_cargo as c ON f.funcionario_id = c.id where f.excluido_em IS NULL LIMIT :limit OFFSET :offset";
